...
def optimize_image(image_path, max_size=(300, 300), quality=70):
    """Optimizes image by resizing and compressing it"""
    try:
        ...
        # Save optimized image back to disk
...
def image_to_base64(image_path):
    """Converts an image file to a Base64 string with optimization"""
    try:
        ...
        # Clean up the optimized file if it's different from original
...
def generate_barcode(name):
    ...
    try:
        ...
        # Configure the writer for smaller images
        writer = ImageWriter()
        ...
        barcode_instance = code128(unique_id, writer=writer)
        full_path = barcode_instance.save(barcode_path)
...
def generate_qr_api():
    ...
    for _ in range(quantity):
        ...
        if not store_qr_in_db(name, unique_id, qr_url, qr_path):
            return jsonify({"isSuccess": False, "message": "Failed to store QR Code in database"}), 500
...
def generate_barcode_api_v2():
    ...
    if not value:
        return jsonify({"isSuccess": False, "message": "Missing required field: value"}), 400
